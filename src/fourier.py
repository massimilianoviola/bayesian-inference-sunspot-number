...
for iter in range(n):
    ...
    if plot_steps:
        ...
        plt.stem(frequencies, amplitudes, linefmt="C7-", basefmt="C1-", markerfmt=" ")
        plt.xscale("log")
        ...
        plt.subplot(3, 1, 2)
        plt.plot(t, current_y, "C7", label="Current signal")
        plt.plot(t, magnitude * np.cos(2 * np.pi / period * t + phase), "C1", label="Estimate")
        #plt.plot(t, B1 * np.cos(2 * np.pi / period * t) + B2 * np.sin(2 * np.pi / period * t), c="C9")
        plt.xlabel("Time")
        plt.ylabel("Normalized count")
        ...
        plt.subplot(3, 1, 3)
        plt.plot(t, current_y - magnitude * np.cos(2 * np.pi / period * t + phase), "C7")
        #plt.plot(t, current_y - B1 * np.cos(2 * np.pi / period * t) - B2 * np.sin(2 * np.pi / period * t), c="C9")
        plt.xlabel("Time")
        plt.ylabel("Normalized count")
        plt.title("Residuals")
...
plt.hist(current_y, color="C7", bins=100)
plt.ylabel("Count")
...
plt.figure(figsize=(12, 4))
plt.plot(t, y - y.mean(), "C7", label="Original")
plt.plot(t, signal, "C1", label="Reconstructed")
plt.xlabel("Time")
plt.ylabel("Normalized count")
plt.title(f"Reconstructed signal using {n} Fourier components")
plt.legend()
plt.tight_layout()
